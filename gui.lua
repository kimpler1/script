-- gui.lua
-- Это библиотека для создания GUI
...
function GUI.createMainGUI()
    ...
    return {
        ...
        isDraggingAnySlider = isDraggingAnySlider -- Экспорт для использования в функциях, если нужно
    }
end

function GUI.createSlider(parent, labelText, toggleFunction, enabledFlag, hasSpeedSlider)
    ...
    if hasSpeedSlider then
        ...
        local function startDrag(input)
            GUI.isDraggingAnySlider = true -- Используем экспортированный флаг
            ...
            if toggleFunction.speedHackEnabled then -- Предполагаем, что toggleFunction может иметь флаги
                toggleFunction.toggleSpeedHack() -- Адаптировать под конкретную функцию
            end
            draggingSpeed = true
            ...
            dragConnection = RunService.RenderStepped:Connect(function()
                ...
                speedValue = math.floor(normalizedPos * 100)
                if toggleFunction.speedHackEnabled then
                    toggleFunction.toggleSpeedHack()
                end
            end)
        end
...
function GUI.createTabsAndSliders(ContentFrame, tabs, sliders, tabButtons, currentTab)
    for i, tabName in ipairs(tabs) do
        ...
        TabButton.Parent = GUI.TabContainer -- Используем из createMainGUI
...
function GUI.applyFixedSettings(ScreenGui, UIPadding, tabButtons, MainFrame, TabContainer, ContentFrame, UIListLayout)
    -- Фиксированные настройки для GUI
